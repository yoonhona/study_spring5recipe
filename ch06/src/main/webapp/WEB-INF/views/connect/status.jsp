...
<c:forEach items="${providerIds}" var="provider">
    ...
    <c:if test="${connectionMap[provider] != null}">
        You are connected to ${provider} as ${connectionMap[provider][0].displayName} <br>
        You are connected to ${provider} as ${connectionMap[provider][0].profileUrl} <br>
        You are connected to ${provider} as ${connectionMap[provider][0]} <br>
    </c:if>
    <c:if test="${connectionMap[provider] == null}">
        <div>
